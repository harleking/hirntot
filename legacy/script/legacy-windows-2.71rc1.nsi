!define INST_VER "002"
!define PRODUCT_NAME "ET-Legacy"
!define PROD_VER "2.71rc1"
...
;!define MUI_FINISHPAGE_RUN_PARAMETERS "+seta com_hunkmegs 128"
;!define MUI_FINISHPAGE_RUN_TEXT "You will find all downloaded files at $ETPATH $DOCUMENTS\WolfETL 'Documents' Folder of Windows"
...
CRCCheck on
  ...
  BrandingText "${PRODUCT_NAME} Version ${PROD_VER} Installer ${INST_VER}"
...
OutFile "..\..\Public\legacy-windows-${PROD_VER}.exe"
...
;For ET update only!
  Function .onVerifyInstDir
    IfFileExists $INSTDIR\ET.exe PathBad
      GOTO CONTINUE
    PathBad:
        Messagebox MB_OK|MB_ICONINFORMATION "Error: Do not install ${PRODuct_NAME} over an existing Wolfenstein - Enemy Territory installation. Please select different Install Folder."
  CONTINUE:
  FunctionEnd
...
Section "${PRODUCT_NAME} ${PROD_VER}" INSTALL
  SectionIn RO
  SetOutPath $INSTDIR
  SetOverwrite try  
  ; Put file there
...
;Omni-Bot
  ...
  SetOutPath "$INSTDIR\legacy\omni-bot\et\gui"
  File "..\${PROD_VER}\legacy\omni-bot\et\gui\*.*"
  ...
  SetOutPath "$INSTDIR\legacy\omni-bot\et\user\download"
  File "..\${PROD_VER}\legacy\omni-bot\et\user\download\*.*"
  ...
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\${PRODuct_NAME}.lnk" "$INSTDIR\etl.exe"
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\${PRODuct_NAME} with Omni-Bots.lnk" "$INSTDIR\etl.exe" "+set omni_bot enable 1 +set omnibot_path legacy\omni-bot\" "$INSTDIR\etl.exe"
...
IfFileExists "$INSTDIR\etmain\pak0.pk3" pak2
...
IfFileExists "$INSTDIR\etmain\pak0.pk3" pak3
...
IfFileExists "$INSTDIR\etmain\pak0.pk3" mp_bin
...
;   Messagebox MB_OK|MB_ICONEXCLAMATION "ETKEY allready exist, nothing to do"
  ...
  NOKEY:
   Messagebox MB_OK|MB_ICONEXCLAMATION "No ETKEY found. ET-Legacy will create new etkey upon start."
  GOTO END  
...
Section "Desktop Shortcut" SHORTCUT
 ...
 ; Shortcuts
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
  CreateShortCut "$Desktop\${PRODuct_NAME}.lnk" "$INSTDIR\etl.exe" "$INSTDIR"
  !insertmacro MUI_STARTMENU_WRITE_END
SectionEnd
...
;  WriteRegStr HKLM "${PRODUCT_UNINST_KEY}" "Publisher" "ACTIVISION"
  ...
  WriteRegStr HKLM "${PRODUCT_UNINST_KEY}" "ReleaseDate" "2013-03-03"
;  WriteRegStr HKLM "${PRODUCT_UNINST_KEY}" "ReleaseCountry" "United States"
  WriteRegStr HKLM "${PRODUCT_UNINST_KEY}" "Titel" "${PRODuct_NAME}"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
	...
	!insertmacro MUI_DESCRIPTION_TEXT ${ASSETS} "If no Installation of Assets found on local disk download Wolfenstein - Enemy Territory Assets. Around 219MB. WITHOUT YOU CAN NOT PLAY!!!"
...
;  Delete "$INSTDIR\etmain\video\etintro.roq"
